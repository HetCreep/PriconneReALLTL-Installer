...
- **Launching the game.** The main **Launch Game** button starts the game through **DMM** — it does **not** update the patch first. For one-click **update + play**, use a shortcut that does: **wrap** a launcher shortcut that points at the game (DMMGamePlayerFastLauncher or a PriconneMultiAccountLauncher account), or — for a **plain DMM** launch — use **Create AutoUpdater Shortcut**. Both of those update the patch first, then launch; wrapping is reversible. (Wrapping a bare DMM shortcut only re-opens the DMM launcher, so DMM users want the AutoUpdater shortcut.)
